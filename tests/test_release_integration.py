...
def test_release_tty_routes_memory_pending_command_without_agent_turn(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    memory = MemoryManager(workspace)
    ...
    state = ScreenState(input="/memory pending --scope project", cursor_offset=31)
    state.selected_slash_index = -1
    args = TtyAppArgs(
        runtime=None,
        tools=ToolRegistry([]),
        model=None,
    ...
    assert state.is_busy is False
    assert len(state.transcript) == 2
    assert entry.id in state.transcript[1].body
    assert "Pending Memory Approvals" in state.transcript[1].body
